...
Published privately at **https://insights.davetaxnz.nz/** on 31 August 2026. Cloudflare Access uses email login codes, an eight-hour session and an exact two-owner allowlist approved by the user and kept in private settings. The Worker also verifies the Access JWT and the same allowlist. Anonymous homepage, catalogue, JavaScript and API requests redirect to Access; forged identity headers do not bypass it; the workers.dev hostname returns 404. The live login screen is verified. The first approved-owner sign-in and authenticated production report check still require the owner to enter their emailed code; do not describe that check as completed yet.

GA4 property `552154177` is connected; the new tag was published and verified on 31 August 2026, alongside the original tag. Completed-day reporting starts from this date, subject to Google's processing delay; actual event ingestion has not yet been confirmed. Live read-only Search Console queries passed in the local backend preview. Blank Access configuration intentionally returns HTTP 503, even for static assets.
...
5. For an initial deployment, deploy the Worker with **no public route** first. The current Wrangler version requires initial secrets together with the deployment: use `wrangler deploy --secrets-file <private-json-file>`; keep that file outside the repository with mode 0600, verify the resulting bindings are `secret_text`, then remove the temporary bundle while preserving the original secured credentials. `workers_dev:false`, `preview_urls:false` and `assets.run_worker_first:true` must remain. Add the one custom domain only after the matching Access policy exists. This sequence has been completed for the current deployment; do not remove its route or Access policy on routine deployments. Do not modify apex, www, MX, SPF, DKIM, DMARC or WordPress records.
