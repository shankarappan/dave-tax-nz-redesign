...
test("production discovery files and legacy redirects are emitted", async () => {
  ...
  const legacySitemapIndex = await readFile(new URL("../dist/client/sitemap_index.xml", import.meta.url), "utf8");
  ...
  const about = await readFile(new URL("../dist/client/about-dave/index.html", import.meta.url), "utf8");
  const studentLoanCategory = await readFile(new URL("../dist/client/category/student-loans/index.html", import.meta.url), "utf8");
  const missingArticleAlias = await readFile(new URL("../dist/client/2026/07/28/resolve-an-overseas-student-loan-default-180k-deal/index.html", import.meta.url), "utf8");
  const january15Archive = await readFile(new URL("../dist/client/2026/01/15/index.html", import.meta.url), "utf8");
  const january13Archive = await readFile(new URL("../dist/client/2026/01/13/index.html", import.meta.url), "utf8");
  const december4Archive = await readFile(new URL("../dist/client/2025/12/04/index.html", import.meta.url), "utf8");
  ...
  assert.match(legacySitemapIndex, /<sitemapindex/);
  assert.match(legacySitemapIndex, /https:\/\/davetaxnz\.nz\/sitemap\.xml/);
  assert.match(booking, /https:\/\/davetaxnz\.nz\/#contact/);
  assert.match(contact, /https:\/\/davetaxnz\.nz\/#contact/);
  assert.match(about, /https:\/\/davetaxnz\.nz\/#about/);
  assert.match(studentLoanCategory, /https:\/\/davetaxnz\.nz\/articles-media\//);
  assert.match(missingArticleAlias, /\/articles-media\/overseas-student-loan-default-deal\//);
  assert.match(january15Archive, /\/articles-media\/student-loan-interest-dave-ananth-analysis\//);
  assert.match(january13Archive, /https:\/\/davetaxnz\.nz\/articles-media\//);
  assert.match(december4Archive, /\/articles-media\/ird-compliance-return-kpi-impact-on-nz-taxpayers\//);
