...
function Header({ menuOpen, setMenuOpen }) {
  ...
  return <header className="site-header">
    ...
    <ActionLink href="https://davetaxnz.nz/book-a-consultation/" external event="consultation_click">Discuss your student loan</ActionLink>
  </header>;
}
...
function Hero() {
  return <section className="hero" aria-labelledby="hero-title">
    <div className="hero__copy"><p className="eyebrow">Dave Ananth · Partner at Meridian Partners · Former Inland Revenue Prosecutor</p><h1 id="hero-title">Student Loan Lawyer NZ and IRD Negotiator</h1><div className="accent-rule" aria-hidden="true" /><p className="hero__lead">Practical legal advice for overseas New Zealanders dealing with long-standing student-loan debt, penalties, repayment demands and possible IRD enforcement.</p>
      ...
      <div className="button-row"><ActionLink href="https://davetaxnz.nz/book-a-consultation/" external event="consultation_click">Discuss your student loan with Dave</ActionLink><ActionLink href={phoneHref} secondary event="call_click"><Phone size={21} weight="fill" /> Call Dave</ActionLink></div>
    </div>
...
function StoriesPreview() {
  ...
  return <section id="stories" className="stories"><div className="section-shell"><div className="stories__heading"><p className="eyebrow eyebrow--gold">Testimonials</p><h2>Student Loan Client Experiences</h2></div><div className="testimonial-grid">{previews.map(([quote, name, date]) => <figure key={name}><blockquote>“{quote}”</blockquote><figcaption><strong>{name}</strong><span>Student loan client · {date}</span></figcaption></figure>)}</div><p className="outcomes-note outcomes-note--light">These are exact excerpts from individual client accounts. Past outcomes do not guarantee future results.</p><div className="story-links"><a className="text-link text-link--light" href={sitePath("testimonials/")}>Read testimonial information <ArrowRight size={18} /></a><a className="text-link text-link--light" href="https://davetaxnz.nz/book-a-consultation/" target="_blank" rel="noopener noreferrer" data-event="consultation_click">Book a free initial call <ArrowRight size={18} /></a></div></div></section>;
}
...
function Contact() {
  return <section id="contact" className="contact"><div className="section-shell contact__grid"><div><p className="eyebrow eyebrow--gold">Contact Dave</p><h2>Discuss your matter with Dave</h2><p>The booking page offers a free 15-minute initial consultation with student-loan and tax-debt enquiry options. Sending an enquiry does not create a lawyer–client relationship or confirm acceptance.</p></div>
    ...
    <div className="contact__action"><Check size={28} weight="bold" /><p>Please do not send extensive confidential, medical, financial or identity information before conflicts are checked and a formal Meridian Partners engagement is confirmed.</p><ActionLink href="https://davetaxnz.nz/book-a-consultation/" external event="consultation_click">Book a free initial call</ActionLink></div>
  </div></section>;
}
...
function PrivacyPage() {
  return <main id="main"><PageIntro eyebrow="Privacy" title="Privacy Statement">Drafted for New Zealand review; final operational details remain intentionally marked for client confirmation.</PageIntro><div className="legal-page section-shell section-pad"><section><h2>Scope</h2><p>DaveTaxNZ must handle personal information in accordance with the New Zealand Privacy Act 2020. This page is a staging draft and is not approved for production.</p></section><section><h2>Information you choose to provide</h2><p>Dave may receive your name, contact details, enquiry type and the information you choose to include when you email, call, use WhatsApp or follow the external booking link. Do not send extensive confidential, medical, financial or identity information before a formal engagement.</p></section><section><h2>Purpose and access</h2><p>Information may be used to respond, complete conflict and suitability checks, arrange a consultation and determine whether Meridian Partners may offer a formal engagement. The authorised recipient, internal access roles and retention period require written confirmation from Dave and Meridian Partners.</p></section><section><h2>Current staging providers</h2><p>The review builds are hosted through OpenAI Sites and GitHub Pages. Google Fonts supplies the displayed web fonts. WhatsApp is an optional external contact channel. Google Analytics is present in the production build configuration but does not run on either staging hostname. The separate booking page remains on the existing DaveTaxNZ website.</p></section><section><h2>Service providers and overseas processing</h2><p className="client-input">Client confirmation required before launch: final production host, booking/form and anti-spam providers, delivery mailbox and email provider, storage locations, authorised users, overseas disclosures, retention periods and deletion settings.</p></section><section><h2>Your rights</h2><p>You may ask to access or correct personal information held about you. The privacy officer name and approved contact pathway require confirmation before launch.</p></section><section><h2>Analytics</h2><p>Google Analytics is configured to run only on the final davetaxnz.nz production hostname. The website event design excludes names, email addresses, phone numbers, message content and other user-entered personal information.</p></section></div></main>;
}

function TestimonialsPage() {
  return <main id="main"><PageIntro eyebrow="Client experiences" title="Testimonials">Only exact, source-approved testimonial wording is published.</PageIntro><div className="legal-page testimonials-page section-shell section-pad"><blockquote>“Within just two days, the entire process was turned around.”</blockquote><p><strong>Jana R</strong><br />Student loan client · June 2025</p><blockquote>“Managed with reassurance, professionalism, trust and kindness.”</blockquote><p><strong>Sarah Williams</strong><br />Student loan client · October 2025</p><blockquote>“His generosity, empathy and kindness.”</blockquote><p><strong>Felicity McKeen</strong><br />Student loan client · April 2025</p><p className="client-input">The full testimonial archive will be added after Dave or Pam confirms the exact source text or screenshots and approves publication on the redesigned site. No wording has been combined.</p><p className="outcomes-note">Testimonials describe individual experiences. Past outcomes do not guarantee future results.</p></div></main>;
}
