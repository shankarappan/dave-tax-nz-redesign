...
function StudentLoans() {
  return <section id="student-loans" className="expertise section-shell section-pad"><div className="section-heading"><p className="eyebrow eyebrow--red">Student Loan Debt &amp; Penalties</p><h2>How Dave helps overseas student-loan borrowers</h2><p>Dave advises on engaging with Inland Revenue, seeking penalty relief where appropriate, presenting repayment or settlement proposals, and assessing possible enforcement or border risks.</p></div>
    <div className="expertise-grid">{expertise.map(({ icon: Icon, title, text }, index) => <div className="expertise-item" key={title}><span className="expertise-item__number">0{index + 1}</span><Icon size={34} weight="light" /><h3>{title}</h3><p>{text}</p></div>)}</div><div className="practice-more"><a className="text-link" href={sitePath("student-loan-negotiations/")}>Read more about student-loan negotiations <ArrowRight size={18} /></a><p><a href={sitePath("student-loan-airport-arrest-warrants/")}>Student-loan airport enforcement</a> · <a href={sitePath("articles-media/three-news-student-loan-reform-dave-ananth/")}>Three News student-loan reform coverage</a></p></div>
  </section>;
}
...
function ArticleCard({ article, internal = true }) {
  ...
  return <article className="article-card"><div className="article-meta"><span>{article.publication}</span><time dateTime={article.isoDate}>{article.date}</time></div><h3>{article.title}</h3><p>{article.summary}</p><span className="article-type">{article.subject} · {article.type}</span><div className="article-actions">{internal && <a className="text-link" href={sitePath(article.path || `articles-media/${article.slug}/`)}>View details <ArrowRight size={18} /></a>}{externalUrl && <a className="text-link text-link--muted" href={externalUrl} target="_blank" rel="noopener noreferrer">Original source</a>}</div></article>;
}
...
function ArticlePage({ article }) {
  const showFeaturedImage = article.image && !article.videoEmbedUrl && !article.suppressFeaturedImage && !article.contentHtml?.includes(`src=\"${article.image}\"`);
  const sourceUrl = article.sourceUrl ?? (!article.contentHtml ? article.url : undefined);
  const readPdf = article.pdf && article.language && article.language !== "English";
  return <main id="main"><PageIntro eyebrow={`${article.subject} · ${article.type}`} title={article.title}>{article.publication} · {article.date}</PageIntro><article className="detail-page section-shell section-pad"><dl><div><dt>Publication</dt><dd>{article.publication}</dd></div><div><dt>Relationship</dt><dd>{article.relationship}</dd></div><div><dt>{article.dateLabel || "Published"}</dt><dd>{article.date}</dd></div></dl>{article.mediaV8 && <p className="media-introduction">{article.summary}</p>}{article.videoEmbedUrl && <figure className={`article-video-wrap${article.videoPresentation === "portrait" ? " article-video-wrap--portrait" : ""}`}><div className="article-video"><iframe src={article.videoEmbedUrl} title={article.videoTitle || article.title} loading="lazy" allow="encrypted-media; picture-in-picture; fullscreen" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /></div>{article.videoCaption && <figcaption>{article.videoCaption}</figcaption>}</figure>}{showFeaturedImage && <img className={`article-featured${article.imagePresentation === "document" ? " article-featured--document" : ""}`} src={article.image} alt={article.imageAlt} width={article.imageWidth} height={article.imageHeight} />}{article.detailDescription && <p>{article.detailDescription}</p>}{article.contentHtml ? <>{!article.mediaV8 && <p className="archive-note">This is general information, not legal, tax or accounting advice.</p>}<div className="article-body" dangerouslySetInnerHTML={{ __html: article.contentHtml }} /></> : <>{!article.detailDescription && <p>{article.summary}</p>}<p>This record identifies the publication and Dave’s relationship to it from the linked source. It does not add or imply any unverified claim.</p></>}<div className="article-source-actions">{article.pdf && <ActionLink href={asset(article.pdf)} external={readPdf} download={!readPdf && (article.pdfDownloadName || true)}>{readPdf ? <><FileText size={20} aria-hidden="true" /> Read PDF</> : "Download the published PDF"}</ActionLink>}{sourceUrl && <ActionLink href={sourceUrl} external>View the original source</ActionLink>}</div><a className="text-link" href={sitePath("articles-media/")}>Return to the archive <ArrowRight size={18} /></a></article></main>;
}
...
export function App({ initialPath = "/" }) {
  ...
  const article = articles.find(item => path === (item.path || `/articles-media/${item.slug}/`).replace(/^\/|\/$/g, ""));
  let content;
